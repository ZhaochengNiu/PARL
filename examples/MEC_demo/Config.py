...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.action_space = [gym.spaces.Discrete(self.total_number_of_edges + 1) for _ in
                             range(0, self.total_number_of_devices)]

        self.act_shape_n = [self.action_dim for _ in range(0, self.total_number_of_devices)]

        self.state_n = None
        # Box(low=-1.0, high=2.0, shape=(3, 4), dtype=np.float32)
        self.observation_space = [gym.spaces.Box(-math.inf, math.inf, shape=(self.total_number_of_edges * 2 + 1 + 2, 1))
                                  for _ in range(0, self.total_number_of_devices)]
